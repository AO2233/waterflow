...
class SARdataset(Dataset):
    # val mode mask = None
    def __init__(self, sar_list, mask_list=None, mode="test", transform=None, normal=False):
        if mode == "test":
    ...
    def __getitem__(self, idx):
        ...
        if self.normal:
            ...
            std_l= [2797.2916273636556, 710.5693234469384, 1173.4579685284687, 342.08553456322215, 17.937904473111903, 17.983585131110953]
            mean_l= [1774.172843321052, 382.70600456948034, 32.83576673379225, 158.46333796412378, 31.623006319089388, 3.7178739697469863]
        ...
        if mask is not None:
            return sar, mask, idx  # idx 为图片在list的绝对标号
        else:
            return sar, idx
...
def build_dataset(dataset, val_rate):
    ...
    val_set.dataset = copy.deepcopy(dataset)
